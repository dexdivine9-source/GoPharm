...
export default function AISearch() {
  ...
  const { createOrder, getAllAvailableInventory } = useSupabase();
  const navigate = useNavigate();

  const handleConfirmOrder = () => {
    const inventory = getAllAvailableInventory();
    if (inventory.length === 0) {
        alert("Sorry, no verified pharmacies currently have this in stock in our active network.");
        return;
    }
    // For the AI demo, we just grab the first available inventory item to fulfill the simulated request
    const item = inventory[0]; 
    createOrder(item.pharmacy_id, item.med_name, 1, item.price);
    navigate('/dashboard');
  };
